...
const ADD_ARTIST_URL = "Artist/add";

const AddArtist = ({ isOpen, closeModal, GetArtists }) => {
  const initialArtist = {
    ArtistName: "",
    ArtistImage: "",
    Description: "",
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(ADD_ARTIST_URL, formData, config);
      if (response !== null) {
        toast.success("add artist successfully!");
        GetArtists();
        setArtist({ ...initialArtist });
        closeModal();
      } else {
        toast.error("Cannot artist user!");
      }
    } catch (error) {
      if (!error?.response) {
  ...
  return (
    <>
      ...
      >
        <div className="pb-5 rounded-lg bg-white lg:min-w-[40rem] min-h-[4rem] lg:h-[35rem] mt-5 overflow-y-auto">
          <div className="flex rounded-t-lg bg-slate-300 h-[2.25rem] items-center justify-between px-3">
            <div className="flex justify-end">Add Artist</div>
            <CloseIcon
