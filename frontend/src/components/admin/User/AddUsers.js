...
const AddUser = ({ isOpen, closeModal, getUsers }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(ADD_USER_URL, formData, config);
      if (response !== null) {
  ...
  return (
    <>
      ...
      >
        <div className="rounded-lg bg-white lg:w-[40rem] min-h-[5rem] lg:h-[30rem] mt-5 pb-5 overflow-y-auto">
          <div className="flex rounded-t-lg bg-slate-300 h-[2.25rem] items-center justify-between px-3">
            <div className="flex justify-end">Add User</div>
            <CloseIcon
          ...
          <form action="#" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              className="my-3 mx-5 float-right bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
            >
              Submit
            </button>
          </form>
