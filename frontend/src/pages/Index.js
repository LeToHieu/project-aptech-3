...
const Index = () => {
  ...
  const {music, isMusic, isPlaying, isMute} = useSelector(state => state.config)
  ...
  const handleTimeLine = (e) => {
    ...
    const span = document.getElementById("audio-player");
    const audio = span.querySelector("audio");
    audio.currentTime = (e.clientX / width.current.clientWidth) * music.duration
  }
  return (
    <>
      <Header />
      <input type="checkbox" id="toggle" />
      <input type="checkbox" id="music-toggle" onClick={() => dispatch(showMusic())}/>
      ...
      {music && (
        ...
        <div className="z-20 absolute bottom-0 bg-zinc-800 h-16 w-full flex items-center">
          ...
          <div className="flex gap-8 items-center ml-5">
            <div>
              <SkipPreviousRoundedIcon sx={{ color: "white", fontSize: 30 }} />
            </div>
            {!isPlaying ? 
              ...
              <div className="flex gap-2 w-fit shrink-0 py-2 pr-5 cursor-pointer mx-auto">
                ...
                <div className="items-center gap-5 flex-1 justify-between px-5 flex ">
                  <div className="w-9 h-9 flex items-center justify-center hover:bg-gray-800 rounded-full">
                    <ThumbDownRoundedIcon fontSize="small" sx={{color: 'white'}} />
                  </div>
                  <div className="w-9 h-9 flex items-center justify-center hover:bg-gray-800 rounded-full">
                    <ThumbUpRoundedIcon fontSize="small" sx={{color: 'white'}} />
                  </div>
                  <div className="w-9 h-9 flex items-center justify-center hover:bg-gray-800 rounded-full">
                    <MoreVertRoundedIcon fontSize="small" sx={{color: 'white'}} />
                  </div>
                </div>
              </div>
            </label>
          ...
          <div className="flex gap-8 items-center mr-5">
            ...
            <div className="relative group">
              {isMute === 0 ?
                <VolumeOffOutlinedIcon sx={{ color: grey[500], fontSize: 25 }} />
                :
                <VolumeUpOutlinedIcon sx={{ color: grey[500], fontSize: 25 }} />
              }
              <div className="absolute top-0 px-2 -left-28 hidden group-hover:block w-40"><input type="range" min={0} max={100} step="any" className="w-20" onChange={(e) => dispatch(showMute(e.target.value))} /></div>
            </div>
            <div><RepeatOutlinedIcon sx={{ color: grey[500], fontSize: 25 }} /></div>
            <div><ShuffleRoundedIcon sx={{ color: grey[500], fontSize: 25 }} /></div>
            {isMusic ? 
              <div onClick={() => dispatch(showMusic())}><ArrowDropUpIcon sx={{ color: "white", fontSize: 40 }} /></div>
              : 
              <div onClick={() => dispatch(showMusic())}><ArrowDropDownIcon sx={{ color: "white", fontSize: 40 }} /></div>
            }
          </div>
        </div>
        </>
